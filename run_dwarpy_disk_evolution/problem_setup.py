...
# transformed coordinates corresponding to given warp
...
path_data = 'dwarpy_data/warped_disk_evolution.rad'

# Open .rad file and save data as an array of dictionaries.
# Each element of the array contains a dictionary with all relevant
...
# To get the disk data at some time step, use:
time_step = 4
disk_data[time_step]

# This time step corresponds to the simulation time of:
get_time = disk_data[time_step]['time']
print(f'Simulation time: {get_time:.1f}')

# To access the other data, use one of the following keys:
print(f'Keys: {disk_data[time_step].keys()}')

# Dictionary keys: The dictionary contains information on the radial location of cell edges 
# (corresponding to 'ri' in the /run_ppdisk_analytic_1/problem_setup.py file), the radial cell 
# centers ('rc'), the stellar mass ('m_star'), the pressure scale height ('hp'), the dimensionless 
# pressure scale height / aspect ratio ('hpr'), the 'time', the radial surface density distribution 
# of dust ('sigmad') and the l vectors for every disk annulus (l is the normal vector of the annulus, 
# respectively).
...
# In the following, some of the disks properties may change in time.
# We will loop over all times an save the relevant RADMC-3D input files 
# for each time step which will later be visualized in an image.

n_steps = len(disk_data)


# The following input files do not change in time and can be created outside
# the loop.
...
# DOES NOT WORK CURRENTLY -> CHECK IF dust_density.inp FILE CAN BE 
# RENAMED FOR mctherm SIMULATIONS
#
# SEE ALTERNATIVE BELOW

"""
# Pre-allocation for arrays in loop.
# We want 3D arrays with shape (nr,ntheta,nphi).
sigmad3d = np.zeros(rr.shape)
hh3d = np.zeros(rr.shape)
hhr3d = np.zeros(rr.shape)

for i_time in tqdm(range(10)):
    #
    # Make warp for each time step
    #
    l = disk_data[i_time]['l']
    ...
    sigmad = disk_data[i_time]['sigmad']   # surface density
    hh = disk_data[i_time]['hh']           # scale height
    hhr = disk_data[i_time]['hhr']         # aspect ratio (equal to hh / rc)
    ...
    #
    with open(f'dust_density_{i_time}.inp','w+') as f:
        f.write('1\n')                       # Format number
...
for i_time in range(n_steps):
    if i_time >= 50:
        print('Remove this if statement if you really want to perform all images.')
        break
    #
    # Make warp for each time step
    #
    ...
    # Write the dust density file
    #
    with open(f'dust_density.inp','w+') as f:
        f.write('1\n')                       # Format number
        f.write('%d\n'%(nr*ntheta*nphi))     # Nr of cells
        f.write('1\n')                       # Nr of dust species
        data = rhod.ravel(order='F')         # Create a 1-D view, fortran-style indexing
    ...
    #
    makeImage(npix=201, incl=0., phi=180., wav=1.65, sizeau=200., setthreads=8, nostar=True)  # this calls RADMC-3D
    im_scat = readImage()
    
    plt.figure()
    plot_image = plotImage(im_scat, au=True, log=True, vmax=-12,vmin=-15, bunit='inu',cmap='hot')
    plot_image['implot'].write_png(f'images/warped_disk_{i_time}.png')
